...
if(ids.length===1){
    ...
    } else if(T.type==='text'&&tedId===T.id&&ted.style.display!=='none'){
      ab=absPos(T);
...
canvas.addEventListener('mousedown',function(e){
  ...
  if(S.penEditId){
    ...
    selOv.insertBefore(mr,selOv.firstChild);
    return;
  }
  if(S.tool==='pen'){
...
canvas.addEventListener('mousemove',function(e){
  ...
  if(S.penEditId&&S.penEditMarquee){
    ...
    var mr=selOv.querySelector('.pen-edit-marquee');
    if(mr){
...
function resizeTed(){
  ...
  if(isArea){
    ...
    ted.style.width='1px';
    ted.style.height='1px';
    var w=Math.max(1,ted.scrollWidth);
    var h=Math.max(1,ted.scrollHeight);
    ted.style.width=w+'px';
...
function openTed(el){
  ...
  ted.style.position='fixed';
  ted.style.left=(ab.x*zoom+S.px+r.left)+'px';
  ted.style.top=(ab.y*zoom+S.py+r.top)+'px';
  ted.style.margin='0';
  ted.style.padding='0';
  ted.style.border='1px dashed #7b61ff';
  ted.style.outline='none';
  ted.style.background='transparent';
  ted.style.boxSizing='border-box';
  ...
  ted.style.fontFamily=el.fontFamily||'system-ui,sans-serif';
  ted.style.fontStyle=el.fontStyle||'normal';
  ted.style.lineHeight=String(lineHVal);
  var align=el.textAlign||'left';
  ted.style.textAlign=(align==='center'||align==='right')?align:'left';
  if(el.letterSpacing!=null&&el.letterSpacing!==''){
    ted.style.letterSpacing=((typeof el.letterSpacing==='number'?el.letterSpacing:parseFloat(el.letterSpacing))*zoom)+'px';
  }else{ted.style.letterSpacing='';}
  ted.style.color=el.fill==='none'?'#000':el.fill;
  ted.value=el.text||'';
  ted.style.width=Math.max(1,(el.w||60)*zoom)+'px';
  ted.style.height=Math.max(1,(el.h||fs*lineHVal)*zoom)+'px';
  var g=document.getElementById('g'+el.id);if(g)g.style.display='none';
  requestAnimationFrame(function(){
    resizeTed();
    ted.focus({preventScroll:true});
...
function reorderLayerInTree(dragId,dropTargetId,dragParentFrame,dragParentGroup,dragRootType,dropParentFrame,dropParentGroup,dropRootType){
  ...
  if(!sameContainer){
    // Cross-root: drop root group onto root frame/el or onto nested row (inside frame/group) -> move group to visual "end of groups"
    var dragIsRootGroup=!dragParentFrame&&!dragParentGroup&&dragRootType==='group';
    var dropIsRootFrameOrEl=!dropParentFrame&&!dropParentGroup&&(dropRootType==='frame'||dropRootType==='el');
    var dropIsNested=!!(dropParentFrame||dropParentGroup);
    if(dragIsRootGroup&&(dropIsRootFrameOrEl||dropIsNested)){
      var rootGroups=S.groups.filter(function(g){return !g.frameId&&!g.groupId;});
      var fromIdx=rootGroups.findIndex(function(x){return x.id===dragId;});
      if(fromIdx<0){ console.log('[reorderLayerInTree] FAIL: drag group not in root groups'); return false; }
      var one=rootGroups.splice(fromIdx,1)[0];
      rootGroups.unshift(one);
      S.groups=S.groups.filter(function(g){return !!g.frameId||!!g.groupId;}).concat(rootGroups);
      console.log('[reorderLayerInTree] OK, moved root group to end (above frames/els)');
      return true;
    }
    if(!dragParentFrame&&!dragParentGroup&&dragRootType==='frame'&&!dropParentFrame&&!dropParentGroup&&(dropRootType==='group'||dropRootType==='el')){
      var rootFrames=S.frames.filter(function(f){return !f.frameId&&!f.groupId;});
      var fromIdxF=rootFrames.findIndex(function(x){return x.id===dragId;});
      if(fromIdxF<0){ console.log('[reorderLayerInTree] FAIL: drag frame not in root frames'); return false; }
      var oneF=rootFrames.splice(fromIdxF,1)[0];
      rootFrames.push(oneF);
      S.frames=S.frames.filter(function(f){return !!f.frameId||!!f.groupId;}).concat(rootFrames);
      console.log('[reorderLayerInTree] OK, moved root frame to end');
      return true;
    }
    if(!dragParentFrame&&!dragParentGroup&&dragRootType==='el'&&!dropParentFrame&&!dropParentGroup&&(dropRootType==='group'||dropRootType==='frame')){
      var rootEls=S.els.filter(function(e){return !e.frameId&&!e.groupId;});
      var fromIdxE=rootEls.findIndex(function(x){return x.id===dragId;});
      if(fromIdxE<0){ console.log('[reorderLayerInTree] FAIL: drag el not in root els'); return false; }
      var oneE=rootEls.splice(fromIdxE,1)[0];
      rootEls.push(oneE);
      S.els=S.els.filter(function(e){return !!e.frameId||!!e.groupId;}).concat(rootEls);
      console.log('[reorderLayerInTree] OK, moved root el to end');
      return true;
    }
    console.log('[reorderLayerInTree] FAIL: parent/root mismatch');
    return false;
  }
  ...
  items.forEach(function(item){
    ...
    d.addEventListener('dragover',function(e){
      ...
      if(ind){
        ...
        if(ind.parentNode!==layersDiv||ind.nextSibling!==d)layersDiv.insertBefore(ind,d);
      }
    });
    ...
    d.addEventListener('drop',function(e){
      ...
      console.log('[Layers drop] reorderLayerInTree result:', ok);
      if(ok){
...
function refreshProps(){
  ...
  bind('ppfill',function(e){T.fill=e.target.value;if(tedId&&T.type==='text'&&tedId===T.id)ted.style.color=T.fill==='none'?'#000':T.fill;rr();var h=document.getElementById('ppfillhex');if(h)h.value=e.target.value;e.target.parentElement.style.background=e.target.value;snapshot();});
  bind('ppfillhex',function(e){T.fill=e.target.value;if(tedId&&T.type==='text'&&tedId===T.id)ted.style.color=T.fill==='none'?'#000':T.fill;rr();snapshot();});
